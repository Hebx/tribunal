...
| `index.ts` | `TribunalMemory` — `remember` / `recall` / `restore` over Quilts, namespaced to the on-chain `memory_ns`, with a self-describing per-quilt manifest. |
...
| `committee.ts` | `Committee` — N models (Kiro gateway, OpenAI-compat) vote in parallel → majority verdict + the config-hash preimage. |
| `env.ts` | Zero-dep `.env` reader for sourcing keys. |

## Operations

- **`remember(entries)`** — Seal-encrypt (hook), batch as one Quilt, embed each
  entry, write to Walrus. Returns the `quiltId` + index rows. Writes a
  `_manifest` entry so `restore` is self-describing.
